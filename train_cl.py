...
from models import Encoder, DecoderWithAttention, Adaptive_Encoder,CaptionFeatureExtractor
...
def main(args):
    ...
    caption_feature_extractor = CaptionFeatureExtractor('bert-base-uncased').to(device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train(train_loader=train_loader,
              ...
              epoch=epoch, extractor=caption_feature_extractor)
        ...
        recent_bleu4 = validate(val_loader=val_loader,
                                ...
                                decoder=decoder,
                                criterion=criterion, 
                                extractor=caption_feature_extractor)
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch, extractor):
    """
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # lables = torch.arange(len(imgs[1])).to(device) 
        # 取出同一张图片的距离和非同一张图片的距离
        # 将labels扩展成batch_size x batch_size的向量
...
def validate(val_loader, encoder, decoder, criterion, extractor):
    """
    ...
    for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
        ...
        # distances_matrix = F.cosine_similarity(imgs[1], targets.unsqueeze(0))
        # 创建标签 维度为batch_size:[0,1,2,3,...]
        # lables = torch.arange(len(imgs)).to(device) 
        # 取出同一张图片的距离和非同一张图片的距离
        # 将labels扩展成batch_size x batch_size的向量
        # match_distances = distances_matrix[lables.view(-1, 1) == lables.view(1, -1)]
        # nonmatch_distances = distances_matrix[lables.view(-1, 1) != lables.view(1, -1)]
        ...
        # References
        allcaps = allcaps.to(sort_ind.device)
